...
public class GUIRoller extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(final int x, final int y) {
        final String containerName = roller.isInvNameLocalized() ? roller.getInvName() : StatCollector.translateToLocal(roller.getInvName());
        fontRenderer.drawString(containerName, (xSize / 2) - (fontRenderer.getStringWidth(containerName) / 2), 4, 4210752);
        fontRenderer.drawString(StatCollector.translateToLocal(TileConstant.INVENTORY), 6, (ySize - 96) + 4, 4210752);
    }
}