...
public class BaseBush extends BlockLeavesBase implements IPlantable {
    ...
    public Icon getIcon(final int side, final int meta) {
        if (this.graphicsLevel) {
            this.blockIcon = BaseBush.bushes[meta].getFancyIcon();
            return this.blockIcon;
        } else {
            this.blockIcon = BaseBush.bushes[meta].getIcon();
            return this.blockIcon;
        }
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(final World world, final int x,
        ...
        if (l < 4)
            return AxisAlignedBB.getBoundingBox(x + 0.25D, y, z + 0.25D, x + 0.75D, y + 0.5D,
                    z + 0.75D);
        else if (l < 8)
            return AxisAlignedBB.getBoundingBox(x + 0.125D, y, z + 0.125D, x + 0.875D, y + 0.75D,
                    z + 0.875D);
        else
    ...
    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool(final World world, final int x,
            final int y, final int z) {
        final int l = world.getBlockMetadata(x, y, z);
        if (l < 4)
            return AxisAlignedBB.getBoundingBox(x + 0.25D, y, z + 0.25D, x + 0.75D, y + 0.5D,
    ...
    @Override
    public void setBlockBoundsBasedOnState(final IBlockAccess iblockaccess, final int x,
            final int y, final int z) {
    ...
    public boolean onBlockActivated(final World world, final int x, final int y, final int z,
        ...
        if (world.isRemote)
            ...
            // final EntityItem entityitem = new EntityItem(world, player.posX,
            // player.posY - 1.0D,
            // player.posZ, new ItemStack(NContent.berryItem.itemID, 1, meta -
            // 12));
            // world.spawnEntityInWorld(entityitem);
            // entityitem.onCollideWithPlayer(player);
            return true;
    ...
    @Override
    public boolean shouldSideBeRendered(final IBlockAccess iblockaccess, final int i, final int j,
    ...
    @Override
    public boolean canSustainPlant(final World world, final int x, final int y, final int z,
    ...
    @Override
    public boolean isFlammable(final IBlockAccess world, final int x, final int y, final int z,
            final int metadata, final ForgeDirection face) {
        return true;
    }

    @Override
    public int getFireSpreadSpeed(final World world, final int x, final int y, final int z,
            final int metadata, final ForgeDirection face) {
        return 4;
    }
