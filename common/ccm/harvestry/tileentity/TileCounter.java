...
public class TileCounter extends TileBase
{
    ...
    public boolean canCut()
    {
        if (this.hasFuel()){
            if (this.recipe.getCuttingResult(this.inventory[0]) != null){
                final ItemStack itemstack = this.recipe.getCuttingResult(this.inventory[0]).getOutput1();
                if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)] == null){
                    return true;
                }
                if (!this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)].isItemEqual(itemstack)){
                    return false;
                }
                final int result = this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)].stackSize + itemstack.stackSize;
                if (this.recipe.getCuttingResult(this.inventory[0]).hasSecondOutput()){
                    final ItemStack itemstack2 = this.recipe.getCuttingResult(this.inventory[0]).getOutput2();
                    if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)] == null){
                        return true;
                    }
                    if (!this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)].isItemEqual(itemstack2)){
                        return false;
                    }
                    final int result2 = this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)].stackSize + itemstack2.stackSize;
    ...
    /**
     * Turn one item from the oven source stack into the appropriate cut item
     * in the counter result stack
     */
    private void cutItem()
    {
        if (this.canCut()){
            final ItemStack itemstack = this.recipe.getCuttingResult(this.inventory[0]).getOutput1();
            if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)] == null){
                this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)] = itemstack.copy();
            }else if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)].isItemEqual(itemstack)){
                this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack)].stackSize += itemstack.stackSize;
            }
            if (this.recipe.getCuttingResult(this.inventory[0]).hasSecondOutput()){
                final ItemStack itemstack2 = this.recipe.getCuttingResult(this.inventory[0]).getOutput2();
                if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)] == null){
                    this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)] = itemstack2.copy();
                }else if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)].isItemEqual(itemstack2)){
