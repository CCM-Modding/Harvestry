...
public class TileCounter extends TileBase {
        ...
        if (this.hasFuel()) {
            if (this.recipe.getCuttingResult(this.inventory[0]) != null) {
                ...
                final int result = this.inventory[InventoryHelper.getBestInventory(this.inventory,
                        3, itemstack)].stackSize + itemstack.stackSize;
                if (this.recipe.getCuttingResult(this.inventory[0]).hasSecondOutput()) {
                    final ItemStack itemstack2 = this.recipe.getCuttingResult(this.inventory[0])
                            .getOutput2();
                    if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3,
                            itemstack2)] == null)
                        return true;
                    if (!this.inventory[InventoryHelper.getBestInventory(this.inventory, 3,
                            itemstack2)].isItemEqual(itemstack2))
                        return false;
                    final int result2 = this.inventory[InventoryHelper.getBestInventory(
                            this.inventory, 3, itemstack2)].stackSize + itemstack2.stackSize;
            ...
            if (this.recipe.getCuttingResult(this.inventory[0]).hasSecondOutput()) {
                ...
                if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)] == null)
                    this.inventory[InventoryHelper.getBestInventory(this.inventory, 3, itemstack2)] = itemstack2
                            .copy();
                else if (this.inventory[InventoryHelper.getBestInventory(this.inventory, 3,
                        itemstack2)].isItemEqual(itemstack2))
    ...
    public void readFromNBT(final NBTTagCompound nbt) {
        ...
        this.setInventory(InventoryHelper.readInventoryFromNBT(
                nbt.getTagList(TileConstants.INVENTORY), TileCounter.invSize));
    }
