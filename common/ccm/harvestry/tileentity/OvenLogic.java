...
public class OvenLogic implements IGUITileLogic {
    ...
    /**
     * Returns true if the oven can grind an item, i.e. has a source item, destination stack isn't full, etc.
     */
    ...
    /**
     * Turn one item from the oven source stack into the appropriate ground item in the oven result stack
     */
    public void run() {
        if (canRun()) {
            final ItemStack itemstack = recipes.getCookingResult(te.getStackInSlot(0)).getOutput1();
