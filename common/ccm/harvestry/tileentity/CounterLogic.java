...
public class CounterLogic implements ITileLogic {
    ...
    public boolean canRun() {
        if (hasFuel()) {
            if (recipes.getCuttingResult(te.getStackInSlot(0)) != null) {
                ...
                final int result = te.getInventory()[InventoryHelper.getBestInventory(te.getInventory(), 3, itemstack)].stackSize + itemstack.stackSize;
                if (recipes.getCuttingResult(te.getStackInSlot(0)).hasSecondOutput()) {
                    ...
                    final int result2 = te.getInventory()[InventoryHelper.getBestInventory(te.getInventory(), 3, itemstack2)].stackSize + itemstack2.stackSize;
                    return (result <= te.getInventoryStackLimit())
    ...
    public void run() {
        if (canRun()) {
            ...
            if (recipes.getCuttingResult(te.getStackInSlot(0)).hasSecondOutput()) {
                ...
                } else if (te.getInventory()[InventoryHelper.getBestInventory(te.getInventory(), 3, itemstack2)].isItemEqual(itemstack2)) {
    ...
    private boolean hasFuel() {
        if ((te.getStackInSlot(1) != null) && (te.getStackInSlot(2) != null) && (te.getStackInSlot(0) != null)) {
            return true;
        } else {
            return false;
        }
