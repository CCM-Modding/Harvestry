...
public class GrinderLogic extends BaseGUILogic {
	...
	public GrinderLogic(TileEntity te) {
		this.te = (ActiveTE) te;
	}
	...
	public boolean canRun() {
		if (te.getStackInSlot(inputSlot) != null) {
			if ((te.getStackInSlot(fuelSlot) != null) && (te.getStackInSlot(fuelSlot2) != null)) {
				if (recipes.getGrindingResult(te.getStackInSlot(inputSlot)) != null) {

					final ItemStack itemstack = recipes.getGrindingResult(te.getStackInSlot(inputSlot))
														.getOutput1();
	...
	public void run() {
		if (canRun()) {
			...
			final ItemStack itemstack = recipes.getGrindingResult(te.getStackInSlot(inputSlot)).getOutput1();
